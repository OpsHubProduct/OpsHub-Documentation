...
* User mention for portal users will not be processed as user mention and its value will be synchronized as text(Value from source).
...
* Limitations due to the lack of Aha! API:
  * Audit/History Sync
    ...
    * **A 5-minute delay is applied to history based sync**
      ...
      * **Current-State-Based Synchronization with Revision History**
        * In current state-based synchronization, if a **getEntityRevisions** utility method is used to retrieve history during mapping, synchronization of event will be in 5-minutes delay from curren time. This happens because aha merges audit records within 5-minute intervals done on the same field.
        * Example Timeline:
        ...
        * Aha end system Audits will be as follows:
          ...
          * Time T2: field2 value changed from **1** to **2**
        * Current sync execution starts at time T5.
        * As part of the history stabilization mechanism, the system will wait for 5 minutes before processing revisions.
        * During this waiting interval, if field2 undergoes another update in the end system, the revision corresponding to field2 may fetch the newer value of the field, whereas the current entity state may still show older value of field2.
        * The latest update for field2 will be captured and synchronized in the subsequent synchronization cycle.
  ...
  * For the below-mentioned entities, it is recommended to configure the following links with the setting:
    **Fail if not found**

...
* **Groups with reserved scopes**: Groups associated with a reserved scope, such as [Team Foundation], can be synchronized only if the required group with same scope already exists in the target system. If it does not exist, processing failures will be observed in <code class="expression">space.vars.OIM</code>.
  * Reason: The end system does not allow to create groups with reserved scopes. For example, if Team Foundation Service Accounts is not available in the target system, the synchronization may fail with the error: "Cannot find this active directory group: 'Team Foundation Service Accounts' in target. Add this group in target system."

...
2. **A <code class="expression">space.vars.OIM</code> user account** with the roles and permissions required for the operations you intend to perform. A dedicated user account is recommended, so that MCP activity is easy to identify in audits and access is limited to only what is needed.

...
The <code class="expression">space.vars.OIM</code> MCP server logs all incoming MCP requests and tool invocations. These logs can be viewed and downloaded directly from the <code class="expression">space.vars.OIM</code> UI - go to the Global Log screen and switch on the **MCP Logs** toggle. Refer to [MCP Logs](../administrator/log-viewer.md#mcp-logs) in the Administration section for details on accessing, downloading, and configuring MCP logs.
...
The `[User:<username>]` and `[ReqId:<correlation_id>]` prefixes identify the user who made the tool call and a unique correlation ID assigned to that call, which is useful for tracing a single call end-to-end when multiple calls are logged concurrently.
...
> **Note**: Credential-like values (passwords, tokens, API keys, secrets) are not present in the logs as-is - they appear masked as `********`.
...
4. In the MCP logs, check whether `Registering MCP tool` entries are present - if absent, the MCP server did not start correctly.
